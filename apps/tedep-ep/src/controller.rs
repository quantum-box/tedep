...
use k8s_openapi::{
  api::core::v1::Namespace, NamespaceResourceScope,
};
use kube::{
  api::ListParams,
      ...
      + Unpin,
{
    ...
    Self: Sized,
  {
    let metrics = Self::Metrics::default();
    {
      ...
      let _ = namespace
        .get(global_config.namespace_str())
        .await?;
    }
    let resources = kube::Api::<Self::Resource>::namespaced(
      client.clone(),
      global_config.namespace_str(),
    );
    ...
    Ok((
      ...
      .run(
        ...
        Arc::new(Self::Context::with_configs(
          &client,
          global_config,
          config,
        )),
    ...
    + Unpin,
{
  ...
  async fn reconcile(
    ...
    let resources = kube::Api::<Self>::namespaced(
      context.client().clone(),
      context.namespace_str(),
    );
    info!(
      "Reconciling {} \"{}\"",
